...
class Event_encoder_block(nn.Module):
    def __init__(self, num_bins, nf):
        ...
        self.conv2_1 = conv_resblock_two(2*nf+self.hidden_dims, 2*nf)
        self.conv3 = conv_resblock_two(2*nf, 4*nf, stride=2)
        # hidden state
        self.hidden_conv = conv_resblock_two(2*nf, self.hidden_dims, stride=1)
...
class Decoder_MS(nn.Module):
    def __init__(self, nf):
        ...
        # level 0
        self.deconv_0_1 = conv_resblock_two(8*nf, 8*nf, stride=1)
        # level 1
        self.deconv_1_1 = deconv4x4(8*nf, 4*nf)
        self.deconv_1_2 = conv_resblock_two(8*nf, 4*nf)
        # level 3
        self.deconv_2_1 = deconv4x4(4*nf, 2*nf)
        self.deconv_2_2 = conv_resblock_two(4*nf, 2*nf)
        # pred image block
...
class EventDeblurNet(nn.Module):
    def __init__(self, num_bins, ETES_module=False):
